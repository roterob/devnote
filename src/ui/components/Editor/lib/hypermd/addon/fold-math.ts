...
export const MathFolder: FolderFunc = (stream, token) => {
  ...
  if (reqAns !== RequestRangeResult.OK) {
    if (reqAns === RequestRangeResult.CURSOR_INSIDE) {
      foldMathAddon.editingExprPosition = { from: expr_from, to: expr_to };
...
export class FoldMath implements Addon.Addon, Options {
  ...
  editingExprPosition: { from: Position; to: Position };
  ...
  constructor(public cm: cm_t) {
    ...
    cm.on("change", () => {
      if (this.editingExpr && this.editingExprPosition) {
        this.checkExpresion();
      }
    });
  }
  ...
  public checkExpresion() {
    if (this.editingExpr && this.editingExprPosition) {
      const { from, to } = this.editingExprPosition;
      var expr: string = this.cm.getRange(from, to).trim();
      if (expr !== this.editingExpr) {
        this.editingExpr = null;
        this.editingExprPosition = null;
      }
    }
  }
}
