...
(function (mod) {
  ...
  var Vim = function () {
      ...
      processSearch: function (cm, vim, command) {
        ...
        function handleQuery(query, ignoreCase, smartCase) {
          ...
          try {
            ...
            showError(cm, "Invalid regex: " + query);
            clearInputState(cm);
            return;
          }
    ...
    function showError(cm, template) {
      var pre = dom("pre", { $color: "red" }, `⛔ ${template}`);
      if (cm.openNotification) {
        cm.openNotification(pre, { bottom: true, duration: 5000 });
      } else {
    ...
    ExCommandDispatcher.prototype = {
      ...
      _processCommand: function (cm, input, opt_params) {
        ...
        if (!commandName) {
          showError(cm, 'Not an editor command ":' + input + '"');
          return;
        }
        try {
    ...
    var exCommands = {
      ...
      map: function (cm, params, ctx) {
        ...
        if (!mapArgs || mapArgs.length < 2) {
          if (cm) {
            showError(cm, `Invalid mapping: ${params.input}`);
          }
          return;
        }
      ...
      global: function (cm, params) {
        ...
        if (regexPart) {
          ...
          try {
            ...
            showError(cm, "Invalid regex: " + regexPart);
            return;
          }
        }
